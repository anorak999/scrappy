...
if (window.__scrappy_injected__) {
  ...
  const waitForDynamicContent = async () => {
    ...
    // Small additional delay for any final renders
  ...
  const preserveFormValues = (doc) => {
    ...
    textareas.forEach(textarea => {
      const originalTextarea = document.querySelector(`textarea[name="${textarea.name}"]`);
      if (originalTextarea) {
    ...
    checkboxes.forEach(input => {
      const selector = `input[type="${input.type}"][name="${input.name}"][value="${input.value}"]`;
